...
public static class SolutionInfoCreator
{
	...
	{
		bool flag = false;
		Solution solution = ((TextDocument)currentSelectedDocument).Project.Solution;
		currentSelectedDocument.GetPossibleProjects();
		List<Project> list = solution.Projects.ToList();
		string text = ((TextDocument)currentSelectedDocument).Project.Name;
		if (text.Contains("("))
		{
			text = text.Substring(0, text.IndexOf("("));
		}
		string projectStartName = ((TextDocument)currentSelectedDocument).Project.Name.Substring(0, text.LastIndexOf("."));

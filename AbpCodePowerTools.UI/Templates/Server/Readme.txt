...
- 前端代码生成的文件文件路径请在@{@data.ReplaceInfo.SolutionNamespace}.Application类库中生成的对应实体代码文件中的Client中。
...
如果你选择了**生成权限功能**，请打开@{@data.ReplaceInfo.SolutionNamespace}.Application类库中的@{@data.ReplaceInfo.SolutionNamespace}ApplicationModule.cs类文件。
然后复制以下代码到 的PreInitialize 方法中:

```csharp
Configuration.Authorization.Providers.Add<@{@data.Entity.Name}AuthorizationProvider>();
...
{
   modelBuilder.ApplyConfiguration(new @{@data.Entity.Name}Cfg());
 }
...
生成的多语言内容
在@{@data.ReplaceInfo.SolutionNamespace}.Core类库中对应实体下的[多语言配置](@{@data.Entity.Name}douyuyan.md)文件中。
